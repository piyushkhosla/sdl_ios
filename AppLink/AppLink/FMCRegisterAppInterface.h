 ...
 * SYNC&reg; before any other interaction with SYNC&reg; can take place. The
 * registration lasts until it is terminated either by the application calling
 * the <i> FMCUnregisterAppInterface</i> method, or by SYNC&reg;
 * sending an <i> FMCOnAppInterfaceUnregistered</i> notification, or
 * by loss of the underlying transport connection, or closing of the underlying
 * message transmission protocol RPC session
 * <p>
 * Until the application receives its first <i>FMCOnHMIStatus</i>
 * Notification, its HMI Status is assumed to be: <i>
 * FMCHMILevel</i>=NONE, <i>
 * FMCAudioStreamingState
 * </i>=NOT_AUDIBLE, <i>
 * FMCSystemContext</i>=MAIN
 * <p>
 * All SYNC&reg; resources which the application creates or uses (e.g. Choice
 * Sets, Command Menu, etc.) are associated with the application's interface
 * registration. Therefore, when the interface registration ends, the SYNC&reg;
 ...
 * will not be able to use the SYNC&reg; HMI without first establishing a new
 * interface registration and re-creating its required SYNC&reg; resources. That
 * is, SYNC&reg; resources created by (or on behalf of) an application do not
 ...
 * <ul>
 * <li>Choice Sets</li>
 * <li>Command Menus (built by successive calls to <i>FMCAddCommand
 * </i>)</li>
 ...
 * between SYNC&reg; and the mobile application is lost, such as the vehicle is
 ...
 *
 * Since AppLink 1.0
 * See FMCUnregisterAppInterface FMCOnAppInterfaceUnregistered
 */
@interface FMCRegisterAppInterface : FMCRPCRequest {}

/**
 * @abstract Constructs a new FMCRegisterAppInterface object
 */
-(id) init;
/**
 * @abstract Constructs a new FMCRegisterAppInterface object indicated by the NSMutableDictionary
 * parameter
