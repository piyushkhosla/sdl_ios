...
-(void) neverCalled {
    ...
    [[FMCOnTBTClientState alloc] release];
    ...
    [[FMCShowConstantTBTResponse alloc] release];
    [[FMCAlertManeuverResponse alloc] release];
...
//        return;
	...
	for (NSObject<FMCProxyListener> *listener in localListeners) {
		if ([listener respondsToSelector:handlerSelector]) {
			...
			[FMCDebugTool logInfo:@"Proxy: Proxy listener doesn't respond to selector: %@", handlerName];
		}
	}
	[localListeners release];
}
