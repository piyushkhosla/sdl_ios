...
@implementation FMCPolicyDataParser
...
- (void)parsePolicyData:(NSData *)data {
    @try {
        ...
        if (self.isHighBandwidth) {
            ...
            if (self.hasESN) moduleMessageIdOffset += self.ESN.length;
            self.moduleMessageId = ntohl(*(UInt32*)(bytes+moduleMessageIdOffset));

            int serverMessageIdOffset = 11;
            if (self.hasESN) serverMessageIdOffset += self.ESN.length;
            self.serverMessageId = ntohl(*(UInt32*)(bytes+serverMessageIdOffset));

            int messageStatusOffset = 15;
            if (self.hasESN) messageStatusOffset += self.ESN.length;
            self.messageStatus = bytes[messageStatusOffset];
        }

        if (self.isEncrypted) {
            int ivOffset = 5;
            if (self.isHighBandwidth) ivOffset += 11;
            if (self.hasESN) ivOffset += self.ESN.length;
            self.initializationVector = [NSData dataWithBytes:(bytes+ivOffset) length:16];
        }

        int payloadOffset = 5;
        if (self.isHighBandwidth) payloadOffset += 11;
        if (self.hasESN) payloadOffset += self.ESN.length;
        if (self.isEncrypted) payloadOffset += self.initializationVector.length;
